...
function UserDirectory() {
    ...
    useEffect(() => {
        ...
        const load = async () => {
            setLoading(true);
            try {
                const data = await apiFetch<Page<User>>(
                    `${API_ENDPOINTS.users}?page=${page}&size=20`,
                );
                                ...
                                >
                                    ...
                                    <td className="px-4 py-3">
                                        <div className="flex justify-end gap-1.5">
                                            {/* Only USER can be promoted to MODERATOR */}
                                            {u.role === "USER" && (
                                                <button
                                                    onClick={() =>
                                                        promote(u.id)
                                            ...
                                            {/* Can't suspend an already-suspended or ADMIN user */}
                                            {!u.isSuspended &&
                                                u.role !== "ADMIN" && (
                                                    <button
                                                        onClick={() =>
