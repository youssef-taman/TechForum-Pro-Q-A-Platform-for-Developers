    ...
    >
      <ReactMarkdown
        ...
        components={{
          code: ({className, children, ...props}) => {
            ...
            if (isBlock) {
              ...
              return (
                <pre
                  className={
                    compact
                      ? "my-2 overflow-x-auto rounded-lg border border-border bg-surface p-2.5 font-code text-xs"
                      : "my-3 overflow-x-auto rounded-lg border border-border bg-surface p-4 font-code text-sm"
                  }
                >
                  <code
                    className={className}
            ...
            return (
              <code
                className={
                  compact
                    ? "rounded bg-surface px-1.5 py-0.5 font-code text-xs text-neon"
          ...
          p: ({children}) => (
            <p className={compact ? "my-1.5" : "my-2"}>{children}</p>
          ),
          ul: ({children}) => (
            <ul
              className={
                compact ? "my-1.5 list-disc pl-5" : "my-2 list-disc pl-6"
              }
            >
              {children}
            </ul>
          ...
          ol: ({children}) => (
            <ol
              className={
                compact ? "my-1.5 list-decimal pl-5" : "my-2 list-decimal pl-6"
              }
            >
              {children}
            </ol>
          ...
          h1: ({children}) => (
            <h3
              className={
                compact ? "mt-2 text-sm font-bold" : "mt-3 text-base font-bold"
              }
            >
              {children}
            </h3>
          ...
          h2: ({children}) => (
            <h4
              className={
                compact ? "mt-2 text-xs font-bold" : "mt-3 text-sm font-bold"
              }
            >
              {children}
            </h4>
