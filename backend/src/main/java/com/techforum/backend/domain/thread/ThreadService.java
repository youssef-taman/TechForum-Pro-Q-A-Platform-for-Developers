...
public class ThreadService {
  ...
  private void checkDuplicateInThreadArchive(float[] threadEmbedding) {
    ...
    Optional<DuplicateThreadDTO> duplicateThread =
        threadRepository.findDuplicateWithThreshold(
            threadEmbedding, THREAD_DUPLICATION_SIMILARITY_THRESHOLD);

    if (duplicateThread.isPresent()) {
      DuplicateThreadDTO duplicateThreadDTO = duplicateThread.get();
      throw new DuplicateThreadException(
          duplicateThreadDTO.threadId(),
          duplicateThreadDTO.authorUsername(),
          duplicateThreadDTO.cosineSimilarityScore());
    }
  }
  ...
  @Transactional
  public ThreadDTO createThread(ThreadCreateDTO threadCreateDTO, Authentication authentication) {
